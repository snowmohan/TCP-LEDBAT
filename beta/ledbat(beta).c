...
// ns2 version takes also this two parameters: u32 rtt, int flag
{
	...
	offset = ((s64) target)*((s64)100000) - (queue_delay);
        offset = offset/((s64)100000);
	offset *= gain_num;
...
*/
	...
	if (offset< ((s64)-1) * target) {
		offset = ((s64)-1) * target;
	}
	ledbat->snd_cwnd_cnt ++;
	if(offset > 0) {
		if(((offset*ledbat->snd_cwnd_cnt)/target) >= tp->snd_cwnd){
			tp->snd_cwnd++;
			ledbat->snd_cwnd_cnt = 0;
		}
	}
        ...
        printk(KERN_DEBUG
               "time %u, queue_delay %lld, offset %lld cwnd_cnt %u, "
               "cwnd %u, delay %lld, min %lld,  max_cwnd %lld\n",
               tcp_time_stamp, queue_delay, offset, ledbat->snd_cwnd_cnt,
               tp->snd_cwnd, current_delay, base_delay, max_cwnd);
#endif
}
/**
				  ...
				  const struct ack_sample *sample)
{
	...
	else if (tcp_time_stamp > (ledbat->last_ack + (tp->srtt_us >> 3))) {
